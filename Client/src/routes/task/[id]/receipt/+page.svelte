...
<main class="min-h-screen bg-[radial-gradient(circle_at_top,rgba(34,211,238,0.13),transparent_22%),linear-gradient(180deg,#020617_0%,#08111f_38%,#020617_100%)] px-6 py-12 text-slate-100">
	<div class="mx-auto max-w-6xl space-y-8">
		<header class="rounded-[2rem] border border-cyan-400/20 bg-slate-950/80 p-8 shadow-[0_30px_90px_rgba(8,145,178,0.15)] backdrop-blur-xl">
			<div class="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
				...
				<div class="flex flex-wrap gap-3">
					<span
						class="rounded-full border px-4 py-2 text-xs font-semibold uppercase tracking-[0.22em] {payoutTone(data.payoutStatus.payout?.status ?? null)}"
					>
		...
		<section class="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
			<div class="space-y-6">
				<article class="rounded-[2rem] border border-slate-800 bg-slate-900/70 p-8">
					<div class="flex flex-col gap-4 border-b border-slate-800 pb-6 md:flex-row md:items-end md:justify-between">
						<div>
							<p class="text-xs uppercase tracking-[0.22em] text-slate-500">Ledger state</p>
					...
					<div class="mt-6 space-y-5">
						<div class="rounded-[1.5rem] border border-slate-800 bg-[linear-gradient(160deg,rgba(8,47,73,0.55),rgba(15,23,42,0.92))] p-6">
							<div class="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
								<div>
									<div class="text-xs uppercase tracking-[0.22em] text-slate-400">Amount</div>
									<div class="mt-3 font-['Space_Grotesk'] text-4xl font-bold text-white">
										{data.task.payoutAmount} {data.task.currencyAsset}
									</div>
								</div>
								<div class="text-right">
									<div class="text-xs uppercase tracking-[0.22em] text-slate-400">Triggered by</div>
									<div class="mt-3 text-sm font-semibold uppercase tracking-[0.16em] text-cyan-200">
						...
						<div class="grid gap-5 md:grid-cols-2">
							<div class="rounded-[1.5rem] border border-slate-800 bg-slate-950/80 p-5">
								<div class="text-xs uppercase tracking-[0.22em] text-slate-500">Task status</div>
								<div class="mt-4 text-lg font-semibold text-white">{data.task.status}</div>
							</div>

							<div class="rounded-[1.5rem] border border-slate-800 bg-slate-950/80 p-5">
								<div class="text-xs uppercase tracking-[0.22em] text-slate-500">Funding confirmed</div>
							...
							<div class="rounded-[1.5rem] border border-slate-800 bg-slate-950/80 p-5">
								<div class="text-xs uppercase tracking-[0.22em] text-slate-500">Worker wallet</div>
								<div class="mt-4 break-all text-sm leading-7 text-cyan-300">
							...
							<div class="rounded-[1.5rem] border border-slate-800 bg-slate-950/80 p-5">
								<div class="text-xs uppercase tracking-[0.22em] text-slate-500">Paid at</div>
				...
				<article class="rounded-[2rem] border border-slate-800 bg-slate-900/70 p-8">
					<div class="flex flex-col gap-4 border-b border-slate-800 pb-6 md:flex-row md:items-end md:justify-between">
						<div>
							<p class="text-xs uppercase tracking-[0.22em] text-slate-500">Task context</p>
							<h2 class="mt-3 font-['Space_Grotesk'] text-3xl font-semibold text-white">
								Workflow trace
							</h2>
						</div>
					...
					<div class="mt-6 grid gap-5 md:grid-cols-2">
						<div class="rounded-[1.5rem] border border-slate-800 bg-slate-950/80 p-5">
							<div class="text-xs uppercase tracking-[0.22em] text-slate-500">Review decision</div>
							<div class="mt-4 text-lg font-semibold text-white">
						...
						<div class="rounded-[1.5rem] border border-slate-800 bg-slate-950/80 p-5">
							<div class="text-xs uppercase tracking-[0.22em] text-slate-500">Submission</div>
							<div class="mt-4 text-sm leading-7 text-slate-200">
						...
						<div class="rounded-[1.5rem] border border-slate-800 bg-slate-950/80 p-5">
							<div class="text-xs uppercase tracking-[0.22em] text-slate-500">Recipient</div>
							<div class="mt-4 text-sm leading-7 text-slate-200">
								{formatWallet(
									data.payoutStatus.payout?.workerWalletAddress ??
										data.payoutStatus.workerWalletAddress
					...
					{#if data.report.latestReviewDecision?.reason}
						<div class="mt-5 rounded-[1.5rem] border border-slate-800 bg-slate-950/80 p-5">
							<div class="text-xs uppercase tracking-[0.22em] text-slate-500">Review note</div>
			...
			<aside class="space-y-6">
				...
				<article class="rounded-[2rem] border border-slate-800 bg-slate-900/70 p-6">
					<h2 class="font-['Space_Grotesk'] text-2xl font-semibold text-white">Verification snapshot</h2>
					{#if data.report.verificationReport}
						<div class="mt-6 space-y-4">
							...
							<div class="rounded-[1.5rem] border border-slate-800 bg-slate-950/80 p-4">
								...
								<div class="mt-3 text-sm font-semibold uppercase tracking-[0.18em] text-white">
				...
				<div class="flex flex-wrap gap-3">
					{#if data.task.status === 'PENDING_REVIEW' && data.session.role === 'client' && data.session.id === data.task.clientId}
						<a
